  ...
  const fetchExistingProgress = async () => {
    if (courseId && token) {
      try {
        ...
        if (response.success && response.data.completedVideos) {
          // Load all previously completed lectures into Redux
          response.data.completedVideos.forEach(videoId => {
            dispatch(updateCompletedLectures(videoId))
          })
        }
      } catch (error) {
        console.error("Failed to fetch existing course progress:", error)
      }
    }
  }
