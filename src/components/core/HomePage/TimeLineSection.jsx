          ...
          {timeline.map((element, index) => (
            <div className="flex flex-row gap-6 relative" key={index}>
              <div className="flex flex-col items-center">
                {/* Logo circle */}
                <div className="w-[50px] h-[50px] bg-white rounded-full flex items-center justify-center z-10">
                  <img src={element.Logo} alt={`${element.heading} icon`} />
                </div>

                {index !== timeline.length - 1 && (
                  <div className="w-px h-[60px] bg-gray-300 border-dashed border-l border-gray-300"></div>
                )}
              </div>
